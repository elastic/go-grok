...
func TestParseWithPatterns_Firewalls(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			map[string]string{
				...
				"destination.ip":      "2001:0db8:85a3:0000:0000:8a2e:0370:7335",
				"iptables.length":     "100",
				"iptables.tos":        "01",
				"iptables.ttl":        "64",
		...
		},
		{
			...
			map[string]string{
				...
				"cisco.asa.original_ip_payload.source.ip":             "192.168.1.1",
				"cisco.asa.original_ip_payload.source.port":           "12345",
				"cisco.asa.original_ip_payload.source.user.name":      "jdoe",
				"cisco.asa.original_ip_payload.destination.ip":        "10.0.0.1",
	...
	for _, tt := range testCases {
		t.Run(tt.Name, func(t *testing.T) {
			g := grok.NewWithPatterns(patterns.Firewalls)
